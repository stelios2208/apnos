...
export function VoiceCuesModal({ uid, lang: appLang, onClose, onChanged }: {
  uid: string;
  lang: string;
  onClose: () => void;
  onChanged?: () => void;
  ...
  useEffect(() => () => { stopStream(); previewRef.current?.pause(); }, [stopStream]);

  const saveBlob = useCallback(async (key: CueKey, blob: Blob) => {
  ...
  const startRecording = useCallback(async (key: CueKey) => {
  ...
  const preview = useCallback((key: CueKey) => {
  ...
  const remove = useCallback(async (key: CueKey) => {
    setBusyKey(key);
    try {
  ...
  const onFile = useCallback((key: CueKey, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (
      ...
      >
        <span
          ...
          style={{ background: has ? "#1D9E75" : "rgba(255,255,255,0.15)", boxShadow: has ? "0 0 6px #1D9E7580" : "none" }}
        />
        ...
        {busy ? (
          ...
          <>
            ...
            <label className="cursor-pointer">
              ...
              <input type="file" accept="audio/*" className="hidden" onChange={(e) => onFile(cue.key, e)} />
            </label>
            {has && (
    ...
    <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: "rgba(0,0,0,0.9)" }}>
      <div className="flex flex-1 flex-col overflow-hidden" style={{ background: "#0a0f1a" }}>
        ...
        <div className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <div>
            <h2 className="text-base font-bold text-white">{el ? "Η φωνή μου" : "My voice cues"}</h2>
            <p className="mt-0.5 text-xs text-white/30">
              {el ? "Ηχογράφησε ή ανέβασε τα δικά σου" : "Record or upload your own"} · {recordedCount}/{CUE_CATALOG.length}
            </p>
          </div>
          <button onClick={onClose} className="rounded-lg p-2 text-white/30 transition-colors hover:text-white">
            <X className="size-5" />
          </button>
        </div>
          ...
          {loading ? (
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="space-y-2">{phases.map((c) => <Row key={c.key} cue={c} />)}</div>
              </div>
              <div>
                <p className="mb-2 text-[0.6rem] font-bold tracking-[0.2em] text-white/25">
                  {el ? "ΟΡΟΣΗΜΑ ΚΡΑΤΗΣΗΣ" : "HOLD MILESTONES"}
                </p>
                <div className="space-y-2">{milestones.map((c) => <Row key={c.key} cue={c} />)}</div>
              </div>
            </div>
          )}
        </div>
...
function IconBtn({ children, onClick, label, active = false, danger = false }: {
