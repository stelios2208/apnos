...
import { type DisciplineCode, type Level, ALL_DISCIPLINES, LEVELS } from "@/lib/athletes";
...
export function AthleteFormModal({ lang, initial, isPending, onSubmit, onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="w-full rounded-t-2xl px-5 pb-10 pt-5" style={{ background: "var(--card)" }}>
        ...
        <div className="mb-5 flex items-start justify-between">
          ...
          <button
            onClick={onClose}
            className="mt-1 text-foreground/30 transition-colors hover:text-foreground"
          >
            <X className="size-5" />
          </button>
        </div>
        ...
        <div className="mb-4 space-y-1.5">
          <label className="text-[0.65rem] font-bold tracking-wider text-foreground/40">
            {lang === "el" ? "ΟΝΟΜΑ" : "NAME"}
          </label>
          <input
            autoFocus
            ...
            className="w-full rounded-xl bg-foreground/5 px-4 py-3 text-sm text-foreground placeholder-white/20 outline-none focus:ring-1 focus:ring-[#1D9E75]"
          />
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {LEVELS.map((l) => (
              <button
                ...
                style={{
                  background: level === l.value ? `${l.color}25` : "rgba(var(--ink),0.04)",
                  border: `1.5px solid ${level === l.value ? l.color : "transparent"}`,
                  color: level === l.value ? l.color : "rgba(var(--ink),0.4)",
                }}
              >
            ...
            {ALL_DISCIPLINES.map((d) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: active ? "rgba(29,158,117,0.2)" : "rgba(var(--ink),0.04)",
                    border: `1.5px solid ${active ? "#1D9E75" : "transparent"}`,
                    color: active ? "#5DCAA5" : "rgba(var(--ink),0.35)",
                  }}
                >
                  {d}
                </button>
        ...
        <button
          ...
          style={{
            background: name.trim() ? "#1D9E75" : "rgba(var(--ink),0.05)",
            color: name.trim() ? "#fff" : "rgba(var(--ink),0.25)",
          }}
        >
