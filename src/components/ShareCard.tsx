...
import {
  type ShareCardData, type ProgramCardData,
  buildShareSvg, buildProgramSvg, svgDataUrl, svgToPngBlob, shareOrDownload,
} from "@/lib/share-card";
...
export function ProgramShareModal({ data, onClose }: { data: ProgramCardData; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col justify-end" style={{ background: "rgba(0,0,0,0.7)" }} onClick={onClose}>
      <div className="rounded-t-3xl p-5" style={{ background: "#0a0f1a" }} onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-bold text-white">{data.lang === "el" ? "Κάρτα προγράμματος" : "Programme card"}</h2>
          <button onClick={onClose} className="rounded-lg p-1.5 text-white/40"><X className="size-5" /></button>
        </div>
        ...
        <p className="mt-2 text-center text-[0.65rem] text-white/30">
          {data.lang === "el" ? "Μοιράσου το μέσω WhatsApp, Instagram, email…" : "Share via WhatsApp, Instagram, email…"}
        </p>
      </div>
    </div>
  );
