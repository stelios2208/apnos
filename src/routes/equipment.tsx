...
const DEFAULTS: { id: string; el: string; en: string; hint_el?: string; hint_en?: string }[] = [
  ...
  { id: "slippers", el: "Παντόφλες", en: "Slippers" },
  {
    ...
    el: "Σαμπουάν",
    en: "Shampoo",
    hint_el: "βοηθά να φορέσεις τη στολή",
    hint_en: "helps slide the wetsuit on",
  },
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-foreground/35">{t("equip.sub")}</p>
        </div>
      </div>
      ...
      >
        <div className="mb-2 flex items-center justify-between">
          ...
          <button
            onClick={resetChecks}
            className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[0.7rem] font-semibold text-foreground/45 hover:text-foreground/70"
        ...
        >
          <div
            ...
            style={{ width: `${pct}%`, background: allDone ? "#5DCAA5" : "#EF9F27" }}
          />
        </div>
      </div>
        ...
        {items.map((it) => {
          ...
          return (
            <div
              ...
              style={{
                background: "var(--card)",
                border: `1px solid ${it.checked ? "rgba(29,158,117,0.3)" : "rgba(var(--ink),0.05)"}`,
              }}
            >
              <div
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full"
                style={{
                  background: it.checked ? "rgba(29,158,117,0.15)" : "rgba(var(--ink),0.04)",
                  color: it.checked ? "#5DCAA5" : "rgba(var(--ink),0.35)",
                }}
              >
                <Icon className="size-4" />
              </div>
              <div className={cn("min-w-0 flex-1", it.checked && "opacity-50")}>
                <p className={cn("text-sm text-foreground", it.checked && "line-through")}>
                  {it.label}
                </p>
                {it.hint && <p className="mt-0.5 text-[0.65rem] text-foreground/35">{it.hint}</p>}
              </div>
              <div
                className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full transition-all"
                style={{
                  background: it.checked ? "#1D9E75" : "transparent",
                  border: it.checked ? "none" : "1.5px solid rgba(var(--ink),0.15)",
                }}
              >
                {it.checked && <Check className="size-3.5 text-foreground" />}
              </div>
              {it.custom && (
                <button
                  onClick={(e) => {
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 rounded-xl bg-foreground/5 px-3 py-2.5 text-sm text-foreground outline-none focus:ring-1 focus:ring-[#1D9E75] placeholder:text-foreground/30"
        />
        <button
          onClick={add}
