...
import {
  ArrowLeft, Play, Pause, SkipForward, X, AlarmClock, Check, Volume2, VolumeX, Vibrate,
  Plus, Pencil, Trash2, ChevronUp, ChevronDown, ArrowUp, ArrowDown, Square, Wind,
  Waves, Flame, Droplet, Moon, Sparkles, ListOrdered, SlidersHorizontal,
} from "lucide-react";
import { useI18n } from "@/lib/i18n";
import {
  type WarmupPreset, type WarmupStep, type WarmupStepKind, type WarmupRound,
  WARMUP_PRESETS, WARMUP_ACCENTS, presetTotalSecs, holdCount, fmtClock,
  loadAlarms, saveAlarms,
  loadCustomWarmups, upsertCustomWarmup, deleteCustomWarmup, newCustomWarmup,
  roundsFromSteps, stepsFromRounds, newRound,
} from "@/lib/warmups";
import {
...
// Ring starts full and shrinks clockwise as the step's remaining time runs out.

function CircularTimer({ progress, color, size = 260, strokeWidth = 8, children }: {
  progress: number; // 0 = step just started, 1 = step finished
  color: string;
  size?: number;
  strokeWidth?: number;
...
function Warmup() {
  ...
  const applyStepFx = useCallback((step: WarmupStep) => {
    ...
    if (!eng) return;
    const phase = step.kind === "hold" ? "hold" : step.kind === "exhale" || step.kind === "rest" ? "recovery" : "breathe";
  ...
  if (preset) {
    ...
    const progress = step.secs > 0 ? 1 - Math.max(0, remaining) / step.secs : 0;
      ...
      <div className="fixed inset-0 flex flex-col select-none" style={{ background: "#070a10" }}>
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-4">
          ...
          <div className="flex flex-col items-center">
            <span className="text-xs font-bold tracking-[0.2em] text-white/40">{lang === "el" ? preset.name_el : preset.name_en}</span>
        ...
        <div className="relative z-10 flex flex-1 flex-col items-center justify-center gap-5">
          <CircularTimer progress={progress} color={color} size={264}>
            <span className="mb-1 flex items-center gap-1.5 text-xs font-bold tracking-[0.3em]" style={{ color }}>
              <StepIcon className="size-3.5" />
              {stepLabel(step.kind, lang).toUpperCase()}
            </span>
            <span className="font-mono text-[3.6rem] font-light leading-none tabular-nums" style={{ color }}>
    ...
    <div className="min-h-screen px-4 pb-24 pt-6" style={{ background: "#070a10" }}>
      ...
      <div className="mt-6 rounded-2xl p-4" style={{ background: "#0d1320", border: "1px solid rgba(255,255,255,0.06)" }}>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {alarms.filter((a) => !ALARM_QUICK.includes(a)).map((secs) => (
            <button
              key={secs}
              onClick={() => toggleAlarm(secs)}
              className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-bold"
              style={{ background: "rgba(239,159,39,0.18)", color: "#EF9F27", border: "1px solid rgba(239,159,39,0.4)" }}
            >
        ...
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            onKeyDown={(e) => { if (e.key === "Enter") addCustomAlarm(); }}
...
function PresetSection({ title, presets, lang, onStart }: {
  title: string; presets: WarmupPreset[]; lang: string; onStart: (p: WarmupPreset) => void;
}) {
...
const KIND_META: Record<WarmupStepKind, { color: string; el: string; en: string; icon: LucideIcon }> = {
...
const ACCENT_ICON: Record<string, LucideIcon> = {
  waves: Waves, wind: Wind, flame: Flame, droplet: Droplet, moon: Moon, sparkle: Sparkles,
};
...
function WarmupBuilder({ initial, lang, onCancel, onSave }: {
  ...
  const addRound = () => setRounds((prev) => [...prev, { breatheSecs: prev.at(-1)?.breatheSecs ?? 120, holdSecs: prev.at(-1)?.holdSecs ?? 60 }]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end" style={{ background: "rgba(0,0,0,0.65)" }} onClick={onCancel}>
      <div className="max-h-[92vh] overflow-y-auto rounded-t-3xl p-5" style={{ background: "#0a0f1a" }} onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mb-2 text-[0.6rem] font-bold tracking-wider text-white/35">{lang === "el" ? "ΕΙΚΟΝΙΔΙΟ" : "ICON"}</p>
          ...
          {WARMUP_ACCENTS.map((a) => {
            ...
            return (
              <button
                ...
                style={{ background: active ? `${a.color}28` : "rgba(255,255,255,0.04)", color: a.color, border: `1.5px solid ${active ? a.color : "transparent"}` }}
              >
        ...
        {mode === "rounds" ? (
          <>
            <p className="mb-2 text-[0.6rem] font-bold tracking-wider text-white/35">{lang === "el" ? "ΓΥΡΟΙ" : "ROUNDS"} · {fmtClock(total)}</p>
            <div className="space-y-2">
              {rounds.map((rnd, i) => (
                <div key={i} className="flex items-center gap-2 rounded-xl p-2.5" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <span className="w-5 shrink-0 text-center text-xs font-bold text-white/25">{i + 1}</span>
                  <RoundPill
                    label={i === 0 ? (lang === "el" ? "Αναπνοή" : "Breathe") : (lang === "el" ? "Ξεκούρ." : "Rest")}
                    color="#5DCAA5"
                  ...
                  />
                  <button onClick={() => delRound(i)} disabled={rounds.length <= 1} className="shrink-0 rounded-lg p-1.5 text-white/20 hover:text-red-400/70 disabled:opacity-20"><Trash2 className="size-3.5" /></button>
                </div>
              ))}
            </div>
            <button
                ...
                return (
                  <div key={i} className="rounded-xl p-2.5" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${meta.color}30` }}>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <button onClick={() => moveStep(i, -1)} disabled={i === 0} className="rounded-lg p-1.5 text-white/30 disabled:opacity-20"><ChevronUp className="size-4" /></button>
                      <button onClick={() => moveStep(i, 1)} disabled={i === steps.length - 1} className="rounded-lg p-1.5 text-white/30 disabled:opacity-20"><ChevronDown className="size-4" /></button>
...
function RoundPill({ label, color, secs, onChange }: {
  label: string; color: string; secs: number; onChange: (secs: number) => void;
}) {
  const [open, setOpen] = useState(false);
  if (open) {
    return (
      <div className="flex flex-1 items-center justify-between rounded-full px-2 py-1" style={{ background: `${color}18`, border: `1px solid ${color}55` }}>
        <button onClick={() => onChange(Math.max(5, secs - 5))} className="px-1.5 text-sm font-bold" style={{ color }}>−</button>
        <span className="font-mono text-xs font-bold" style={{ color }}>{fmtClock(secs)}</span>
        <button onClick={() => onChange(secs + 5)} className="px-1.5 text-sm font-bold" style={{ color }}>+</button>
        <button onClick={() => setOpen(false)} className="ml-1 rounded-full px-1.5 text-[0.6rem] font-bold" style={{ color }}>✓</button>
      </div>
    );
  }
  return (
    ...
    >
      <span className="text-[0.5rem] font-bold tracking-wider" style={{ color: `${color}bb` }}>{label.toUpperCase()}</span>
      <span className="font-mono text-xs font-bold" style={{ color }}>{fmtClock(secs)}</span>
    </button>
  );
}
