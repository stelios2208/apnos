...
function isPool(d: string) {
  return POOL_DISC.includes(d as DisciplineCode);
}
function isDepth(d: string) {
  return DEPTH_DISC.includes(d as DisciplineCode);
}

// month key → "YYYY-MM"
function monthKey(dive: Dive): string {
  return dive.dive_date.slice(0, 7);
}
...
function History() {
  ...
  const [discFilter, setDiscFilter] = useState<DisciplineCode | null>(search.disc ?? null);
  ...
  const bulkDelete = async () => {
    ...
    const msg =
      lang === "el" ? `Διαγραφή ${targets.length} βουτιών;` : `Delete ${targets.length} dives?`;
    ...
    () =>
      dives.filter((d) => {
        ...
        if (searchQuery) {
          ...
          if (!d.discipline.toLowerCase().includes(q) && !d.notes?.toLowerCase().includes(q))
            return false;
        }
        return true;
      }),
  ...
  const activeFilters =
    (sessionFilter !== "all" ? 1 : 0) + (fedFilter !== "all" ? 1 : 0) + (discFilter ? 1 : 0);
    ...
    <div className="space-y-4 pb-24">
        ...
        <div className="flex shrink-0 gap-2">
          ...
          {dives.length > 0 && (
            <div
              ...
              style={{ background: "var(--card)", border: "1px solid rgba(var(--ink),0.08)" }}
            >
              <button
              ...
              >
                <CalendarDays className="size-3.5 text-foreground/70" />
              </button>
              <button
              ...
              >
                <LayoutList className="size-3.5 text-foreground/70" />
              </button>
            </div>
          )}
          {dives.length > 0 && (
          ...
          {dives.length > 0 && (
            <button
              ...
              style={{
                background: selectMode ? "rgba(239,80,80,0.12)" : "var(--card)",
                border: `1px solid ${selectMode ? "rgba(239,80,80,0.35)" : "rgba(var(--ink),0.08)"}`,
              }}
            >
              <ListChecks
                className="size-4"
                style={{ color: selectMode ? "#ef5050" : "rgba(var(--ink),0.6)" }}
              />
            </button>
          )}
          {dives.length > 0 && (
            <button
              ...
              style={{ background: "var(--card)", border: "1px solid rgba(var(--ink),0.08)" }}
            >
              <SlidersHorizontal className="size-4 text-foreground/60" />
              {activeFilters > 0 && (
                <span
                  className="absolute -right-1 -top-1 flex size-4 items-center justify-center rounded-full text-[0.55rem] font-bold text-foreground"
      ...
      {dives.length > 0 && (
        <div
          ...
          style={{ background: "var(--card)", border: "1px solid rgba(var(--ink),0.07)" }}
        >
          <Search className="size-4 shrink-0 text-foreground/25" />
          <input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder={
              lang === "el" ? "Αναζήτηση (π.χ. STA, AIDA)…" : "Search (e.g. STA, notes)…"
            }
            className="flex-1 bg-transparent text-sm text-foreground outline-none placeholder-white/20"
          />
          {searchQuery && (
            <button
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {isOpen ? (
                    <ChevronDown className="size-4 text-foreground/30" />
                  ) : (
                ...
                {isOpen && (
                  <div
                    ...
                    style={{ borderColor: "rgba(var(--ink),0.05)" }}
                  >
                    {monthDives.map((dive) => (
                      <DiveCard
                        key={dive.id}
      ...
      {selectMode && (
        <div
          ...
          style={{
            ...
            borderColor: "rgba(239,80,80,0.25)",
        ...
        >
          <div className="mx-auto flex max-w-2xl items-center gap-2">
            <button
              ...
              style={{ background: "rgba(var(--ink),0.05)", color: "rgba(var(--ink),0.6)" }}
            >
              {allFilteredSelected
                ? lang === "el"
                  ? "Καθαρισμός"
            ...
            <button
              ...
              style={{ background: "rgba(var(--ink),0.05)", color: "rgba(var(--ink),0.55)" }}
            >
      ...
      {compareDives.length > 0 && (
        <CompareBar dives={compareDives} onClear={() => setCompareIds([])} lang={lang} />
      )}
...
function EnvironmentSection({
  ...
  dives: Dive[];
  discFilter: DisciplineCode | null;
  onDiscFilter: (c: DisciplineCode) => void;
  expanded: Set<string>;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-1.5">
        {disciplines
          .filter((c) => dives.some((d) => d.discipline === c))
          .map((c) => (
            <button
              key={c}
...
function SubSection({
  ...
  return (
    <div className="space-y-2 pl-3">
      <div className="flex items-center gap-2">
        <span className="text-[0.6rem] font-semibold tracking-[0.18em] text-foreground/35">
          {labelEl}
        </span>
        <span
          className="rounded-full px-1.5 py-0.5 text-[0.55rem] font-bold"
          style={{ background: "rgba(var(--ink),0.06)", color: "rgba(var(--ink),0.4)" }}
        >
          {dives.length}
        </span>
      </div>
...
function DiveCard({
  ...
  return (
    <li
      ...
      style={{
        background: isSelected ? "rgba(239,80,80,0.06)" : "var(--card)",
        border: isSelected
          ? "1px solid rgba(239,80,80,0.5)"
          : isComparing
            ? `1px solid ${border}`
            : "1px solid rgba(var(--ink),0.06)",
    ...
    >
        ...
        {selectMode && (
          ...
          >
            {isSelected ? (
              ...
              <Circle className="size-5 text-foreground/25" />
            )}
          </button>
        )}
        <span
      ...
      <button
        onClick={() => {
          if (selectMode) selection?.toggle(dive.id);
          else onToggle();
        }}
      ...
      >
        ...
        {isExpanded ? (
          <ChevronDown className="size-3.5 text-foreground/25" />
        ) : (
          <ChevronRight className="size-3.5 text-foreground/25" />
        )}
      </button>
      ...
      {isExpanded && (
        ...
        >
          {(dive.sleep_hours != null || dive.mental_state != null) && (
            <div className="flex flex-wrap gap-4 text-xs text-foreground/45">
          ...
          {dive.food_notes && (
            <p className="text-xs text-foreground/40">
              <span className="font-medium text-foreground/60">{t("hist.foodLabel")}</span>{" "}
              {dive.food_notes}
            </p>
          )}
          {dive.notes && (
            <p className="text-xs text-foreground/40">
              <span className="font-medium text-foreground/60">{t("hist.notesLabel")}</span>{" "}
              {dive.notes}
            </p>
          )}
...
function CompareBar({
  ...
  return (
    <div className="fixed inset-x-0 bottom-16 z-50 mx-auto max-w-2xl px-4">
      <div
      ...
      >
        {dives.length === 1 ? (
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClear} className="text-foreground/30 hover:text-foreground/60">
              <X className="size-4" />
            </button>
          </div>
        ) : (
          <div className="space-y-3">
              ...
              {[a, b].map((dive) => (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `3px solid ${cardBorder(dive)}`,
                  }}
                >
                  <p className="text-[0.6rem] font-bold tracking-wider text-[#5DCAA5]">
                    {dive.discipline}
                  </p>
            ...
            {a.discipline === b.discipline && (
              ...
              >
                {formatDiff(a, b)} {lang === "el" ? "διαφορά" : "difference"}
              </div>
            )}
          </div>
        )}
...
function EmptyState({ t }: { t: (k: string) => string }) {
  return (
    ...
    >
      ...
      <p className="mt-4 font-semibold text-foreground">{t("hist.empty")}</p>
      <p className="mt-1 text-sm text-foreground/40">{t("hist.emptySub")}</p>
...
function FilterEmpty({ lang, onReset }: { lang: string; onReset: () => void }) {
  return (
    <div
      className="flex flex-col items-center rounded-2xl border border-foreground/06 p-10 text-center"
      style={{ background: "var(--card)" }}
    >
      ...
      <button onClick={onReset} className="mt-4 text-xs font-semibold" style={{ color: "#5DCAA5" }}>
