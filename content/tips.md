# Apnos — Περιεχόμενο «Συμβουλές & Νους»

Επεξεργάσου ελεύθερα τα κείμενα εδώ. **ΜΗΝ αλλάξεις** τις γραμμές `### [id] · category` —
τις χρησιμοποιώ για να ξαναβάλω τις αλλαγές στην εφαρμογή (`src/lib/tips.ts`).
Όταν τελειώσεις, πες μου «σύγχρονισε τα tips» και τα περνάω στον κώδικα.

Κατηγορίες: `safety` (Ασφάλεια) · `eq` (Εξίσωση) · `mental` (Νους) · `relax` (Χαλάρωση) · `technique` (Τεχνική).
Το `premium: true` σημαίνει κάρτα για μελλοντικό paid tier (τώρα δωρεάν).
